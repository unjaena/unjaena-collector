...
3. RAWImageBackend - RAW/DD image file, including numbered split RAW sets
...
Usage:
    ...
    # Split RAW image
    with RAWImageBackend("disk.001") as disk:
        mbr = disk.read(0, 512)
...
def _is_numbered_raw_segment(path: Path) -> bool:
    """Return True for first-part numbered RAW image segments such as .001."""
    suffix = path.suffix.lower()
    return len(suffix) == 4 and suffix.startswith('.') and suffix[1:].isdigit()


def _find_numbered_raw_segments(first_segment: Path) -> List[Path]:
    """Find contiguous numbered RAW segments starting from a selected segment."""
    if not _is_numbered_raw_segment(first_segment):
        return [first_segment]

    try:
        start_index = int(first_segment.suffix[1:])
    except ValueError:
        return [first_segment]

    prefix = str(first_segment)[:-4]
    segments: List[Path] = []
    index = start_index

    while index <= 999:
        candidate = Path(f"{prefix}.{index:03d}")
        if not candidate.exists():
            break
        segments.append(candidate)
        index += 1

    return segments or [first_segment]
...
class RAWImageBackend(UnifiedDiskReader):
    ...
    Features:
    - Simple file I/O
    - Supports .dd, .raw, .img, .bin, .001 split RAW sets, etc.
    - Memory mapping option for single-file RAW images
    ...
    def _open(self):
        ...
        try:
            first_path = Path(self.image_path)
            self._segment_paths = _find_numbered_raw_segments(first_path)
            logger.info(
                "Opening RAW image: %s (%d segment%s)",
                self.image_path,
                len(self._segment_paths),
                "" if len(self._segment_paths) == 1 else "s",
            )

            offset = 0
            for segment_path in self._segment_paths:
                fh = open(segment_path, 'rb')
                fh.seek(0, 2)
                segment_size = fh.tell()
                fh.seek(0)

                self._files.append(fh)
                self._segment_starts.append(offset)
                self._segment_sizes.append(segment_size)
                offset += segment_size

            if not self._files:
                raise DiskError(f"No RAW segments found: {self.image_path}")

            self._file = self._files[0]
            self._disk_size = offset

            # Memory mapping (optional)
            if self._use_mmap and len(self._files) == 1 and self._disk_size > 0:
                import mmap
            ...
            elif self._use_mmap and len(self._files) > 1:
                logger.debug("Memory mapping disabled for split RAW images")
    ...
    def read(self, offset: int, size: int) -> bytes:
        ...
        if offset < 0:
            raise DiskReadError(f"RAW read failed: negative offset {offset}")
        if size <= 0 or offset >= self._disk_size:
            return b''
        ...
        try:
            ...
            remaining = min(size, self._disk_size - offset)
            current_offset = offset
            chunks = []

            segment_index = bisect.bisect_right(
                self._segment_starts, current_offset
            ) - 1
            if segment_index < 0:
                segment_index = 0

            while remaining > 0 and segment_index < len(self._files):
                segment_start = self._segment_starts[segment_index]
                segment_size = self._segment_sizes[segment_index]
                segment_offset = current_offset - segment_start
                if segment_offset < 0:
                    segment_offset = 0

                available = segment_size - segment_offset
                if available <= 0:
                    segment_index += 1
                    continue

                to_read = min(remaining, available)
                fh = self._files[segment_index]
                fh.seek(segment_offset)
                ...
                if read_len < to_read:
                    break
                if segment_offset + read_len >= segment_size:
                    segment_index += 1
