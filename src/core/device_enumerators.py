...
class AndroidDeviceEnumerator(BaseDeviceEnumerator):
            ...
            for dev_info in connected:
                ...
                device = UnifiedDeviceInfo(
                    ...
                    metadata={
                        ...
                        'security_patch': getattr(dev_info, 'security_patch', ''),
    ...
    def _android_capability_summary(self, is_rooted: bool) -> dict:
        """Summarize Android artifact availability for the current privilege level."""
        try:
            from collectors.android_collector import ANDROID_ARTIFACT_TYPES
        except Exception:
            return {}

        implemented = len(ANDROID_ARTIFACT_TYPES)
        dual_mode = 0
        root_only = 0
        nonroot = 0

        for info in ANDROID_ARTIFACT_TYPES.values():
            is_dual = 'root' in info and 'nonroot' in info
        ...
        available = implemented if is_rooted else implemented - root_only
        return {
            'implemented_artifacts': implemented,
            'available_artifacts': available,
            'root_only_artifacts': root_only,
            'nonroot_artifacts': nonroot,
            'dual_mode_artifacts': dual_mode,
            'collection_model': 'root_full' if is_rooted else 'nonroot_limited',
        }
...
class ForensicImageEnumerator(BaseDeviceEnumerator):
    ...
    def register_image(self, file_path: str) -> UnifiedDeviceInfo:
        ...
        raw_segments = (
            self._find_raw_segments(path)
            if device_type == DeviceType.RAW_IMAGE and self._is_numbered_raw_segment(path)
            else []
        )
        ...
        if device_type == DeviceType.E01_IMAGE:
            ...
            size_bytes = sum(Path(segment).stat().st_size for segment in raw_segments)
        ...
        device = UnifiedDeviceInfo(
            ...
            metadata={
                'file_path': str(path),
                'extension': ext,
                'segments': self._find_e01_segments(path) if device_type == DeviceType.E01_IMAGE else raw_segments,
            ...
            for p in partitions:
                ...
                if priority > fs_priority.get(best_fs, 0):
                    ...
                    if fs in ('NTFS', 'FAT32', 'exFAT', 'FAT16', 'FAT12'):
                        best_os = 'windows'
                    elif fs in ('ext2', 'ext3', 'ext4', 'XFS', 'Btrfs'):
                        best_os = 'linux'
    ...
    def _find_e01_segments(self, first_segment: Path) -> List[str]:
        ...
        # E01 -> E02, Ex01 -> Ex02, L01 -> L02, Lx01 -> Lx02, S01 -> S02.
    ...
    def _is_numbered_raw_segment(self, path: Path) -> bool:
        """Return True for numbered RAW image segments such as .001."""
        suffix = path.suffix.lower()
        return len(suffix) == 4 and suffix.startswith('.') and suffix[1:].isdigit()

    def _find_raw_segments(self, first_segment: Path) -> List[str]:
        """Find contiguous numbered RAW segments."""
        if not self._is_numbered_raw_segment(first_segment):
        ...
        while index <= 999:
            ...
            if not candidate.exists():
                break
            segments.append(str(candidate.resolve()))
            index += 1

        return segments or [str(first_segment)]
...
class MobileFFSBundleEnumerator(BaseDeviceEnumerator):
    ...
    def register_bundle(self, file_path: str) -> UnifiedDeviceInfo:
        ...
        size_bytes = path.stat().st_size
        present_artifacts, present_counts, present_scan_complete = (
            self._scan_present_artifacts(path)
        )
        info = UnifiedDeviceInfo(
            ...
            metadata={
                ...
                "present_artifact_count_by_type": present_counts,
                "present_artifact_scan_complete": present_scan_complete,
            },
        )
    ...
    def _scan_present_artifacts(self, path: Path) -> tuple:
        """Return artifact types actually present in a supported FFS bundle."""
        try:
            from collections import Counter
            from collectors.mobile_ffs import CellebriteAdapter

            counts = Counter()
            with CellebriteAdapter(path) as adapter:
